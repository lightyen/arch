...
if pacman -Q xorg-server 1>/dev/null 2>&1; then
	...
	if dmesg | grep 'Hypervisor detected' 1>/dev/null; then
		runuser -l $username -c "curl -fsSL https://raw.githubusercontent.com/lightyen/arch/main/vbox/.xprofile -o ~/.xprofile"
	else
		runuser -l $username -c "curl -fsSL https://raw.githubusercontent.com/lightyen/arch/main/.xprofile -o ~/.xprofile"
	fi
fi
